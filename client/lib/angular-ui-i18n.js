...
(function(angular, deepExtend) {
      ...
      compile: function() {
        return {
          pre: function($scope, $elm, $attrs) {
            ...
            var setValue = function(evt, lang) {
              if (observer) {
            ...
            var listener = $scope.$on(UPDATE_EVENT, setValue);
            setValue(null, langCache.current);
  ...
  uiI18n.provider('i18nService', function i18nServiceProvider() {

    this.api = uiI18nApi;

    this.$get = function i18nServiceFactory() {
...
})(angular, function deepExtend(dst) {
  'use strict';
